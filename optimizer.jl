...
function crossentropy(f, x0, ϵ)
    ...
    return optimresult(μ, 0.0, i, genpop * i, f(μ))
end
...
function plot_results(results)
    ...
    ylabel("Performance (% of Nelder-Mead Value)")
    title("Cross-Entropy Optimization Performance")
    grid(true)
    show()
end
# benchmarking script - quick n dirty
# begin
#     global prob = OptimProblemStatement(3, [-30.0 30.0; -30.0 30.0; 0.0 2.0], "Cross-Entropy", Vector{Vector{Float64}}(), 5.0)
    ...
    # Initialize rewards and timings
    # nelder_mead_rewards = []
    # cross_entropy_rewards = []
    # nelder_mead_times = []
    # cross_entropy_times = []
    ...
    #     for j in 1:10  # Agents
    #         # Run Cross-Entropy Optimization
    #         x0 = randInDomain()
    #         ce_time = @elapsed begin
    #             result = crossentropy(reward, x0, ϵ)
    #         end
    #         push!(cerewards, result.fnval)
    #         push!(cetimes, ce_time)
            
    #         # Run Nelder-Mead Optimization
    #         nm_time = @elapsed begin
    #             result = optimize(reward, x0, NelderMead())
    #         end
    #         push!(nmrewards, result.minimum)
    #         push!(nmtimes, nm_time)
    #     end
        
    #     push!(nelder_mead_rewards, mean(nmrewards))
    #     push!(cross_entropy_rewards, mean(cerewards))
    #     push!(nelder_mead_times, mean(nmtimes))
    #     push!(cross_entropy_times, mean(cetimes))
    # end
    # # Function to plot cumulative rewards
    # function plot_cumulative_rewards(ce_rewards, nm_rewards)
    #     figure()
    #     boxplot([-ce_rewards, -nm_rewards], labels=["Cross-Entropy Rewards", "Nelder-Mead Rewards"])
    ...
    # function plot_execution_times(ce_times, nm_times)
    #     figure()
    #     boxplot([ce_times, nm_times], labels=["Cross-Entropy Time", "Nelder-Mead Time"])
    ...
    # # Call the functions with the results
    # plot_cumulative_rewards(cross_entropy_rewards, nelder_mead_rewards)
    # plot_execution_times(cross_entropy_times, nelder_mead_times)
# end
